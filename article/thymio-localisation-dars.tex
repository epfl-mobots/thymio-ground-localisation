...
However, while several algorithms in this field require global localization, this capability is not yet available in low-cost robots without external aids.
...
This provides a crucial piece on which to build on, for instance to implement navigation or behavioural algorithms.
}
...
This paper answers this need by providing a system using only approximate dead-reckoning and inexpensive infrared sensors measuring the grayscale intensity of the ground, without knowing the initial pose of the robot.
...
This problem can be alleviated by detecting a known pattern projected on the ceiling and fusing this information with odometry.
...
Yet, this approach requires the ground to be equipped with tags which can become expensive and tedious to deploy.
...
To the best of our knowledge, existing systems for localization of multi-robot systems either rely on an expensive adaptation of the environment using markers or beacons, or necessitate the embedding of expensive sensing capabilities on every robot.
In contrast, our system it affordable since it only requires a low number of inexpensive infrared sensors onboard the robot (which are typically used as cliff detectors), and a simple grayscale pattern on the ground that can be achieved using a printed poster.
...
Similarly, using \texttt{trajectory with kidnapping}, we found the value for $p_\mathrm{uniform}$ to be in the order of 0.1.
...
All angular discretizations provide equal position precision.
...
In this paper, we have conducted an empirical evaluation of Markov- and Monte Carlo-based approaches for localizing mobile robots on a known ground visual pattern.
