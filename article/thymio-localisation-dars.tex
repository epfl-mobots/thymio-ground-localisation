...
Therefore, we propose to compute $p_\mathrm{correct}$ from $\sigma_\mathrm{obs}$ by considering a sensor reading black on a black ground:
...
Our robot moves at 3\,cm/s with a time step of 0.3\,s and 3\,cm cells in which the color is known to be similar; this yields $\ent{loss,d}=0.33$ (\eq{H_loss}).
...
Computing the value of $p_\mathrm{correct}$ to match the observed convergence distance by inverting the computation, we find 0.97.
This means that the actual value of $p_\mathrm{correct}$ in our setup would be at least 0.97 but most probably higher such that the theoretical convergence distance is smaller than 20\,cm.

To be consistent in the observation model, it would yield a value of $\sigma_\mathrm{obs}$ smaller than 0.26.
If we use $\sigma_\mathrm{obs} = 0.15$ as measured on grayscale images, we have $p_\mathrm{correct} = 0.99957$.
This leads to a minimum localization distance of about 14.3\,cm.
This value is slightly lower than our experimental results, which makes sense as it is a lower bound.
Moreover, it would be attained with a perfect filter observing a perfect pattern.
Our filter is not perfect, because we have overestimated $\sigma_\mathrm{obs}$.
