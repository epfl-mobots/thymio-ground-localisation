...
From this value, we can compute $p_\mathrm{correct}$, the probability that the value measured by the sensor corresponds to the one in the map.
Considering a sensor reading black on a black ground, we have $p_\mathrm{correct} = p(Z=0|X<0.5) = \int_{-\infty}^{0.5} \mathcal{N}(0,0.5) = 0.84$.
...
\textbf{Experimental application}. In our example, we have assumed $p_\mathrm{correct}=0.84$, that is $\ent{noise}=0.63$.
...
As such the robot gathers on average at most 0.030\,bit per time step, or 0.034\,bit/cm.
...
This means that, on average, the robot should not be localized before traveling around 610\,cm.
This distance is far larger than the observed one.
The reason is that, in our setup, $p_\mathrm{correct}$ is higher than 0.84.
If instead of 0.84, we take $p_\mathrm{correct} = 0.97$ (corresponding to $\sigma_\mathrm{obs} = 0.26$), then $\ent{noise}=0.19$.
With this value, the robot gathers on average at most 0.9\,bit per time step, or 1.0\,bit/cm, and should not be localized before traveling around 20\,cm.
The short localization distance despite an overestimated $\sigma_\mathrm{obs}$ shows that our algorithm is robust to imprecision in its parameters.
...
In the context of distributed collective robotics, they provide a solid base on which to build navigation or behavioral algorithms.
