...
Cheaper products such as the Thymio~II (120\,\$) and Dash \& Dot (160\,\$) even lack such long-range perception capability, and only provide local infrared sensors.
...
While these approaches are commonly deployed in robots with extensive sensing capabilities such as laser scanners, their implementation on extremly low bandwidth sensors is novel, and rises specific questions, such as which distance the robot must travel for the localisation to converge.
...
This project uses only its two ground infrared sensors (\fig{thymio}, right) and the measurement of the back electromotive force, which is proportional to the speed of the wheels.
...
This value is chosen so that at maximum recorded speed, the robot travels approximately the length of one cell.
