...
However, these robots have little long-range sensing abilities, as they are still more expensive than shorter range ones, and bad dead-reckoning capabilities.
...
Cheaper products such as the Thymio~II (130\,\$) and Dash \& Dot (160\,\$) even lack such long-range perception capability, and only provide local infrared sensors.
These are limited to sensing distances up to 15\,cm and measuring the grayscale intensity of a close object or the ground.
The even cheaper mBot (75\,\$), based on an Arduino board, is equipped with a long range ultrasound sensor, but this is nearly the only exteroceptive sensors available.
...
A notable exception is the BeeBot (90\,\$), which uses proper dead-reckoning as its only sensor.
% But this robot can only follow pre-programmed trajectories, it is not able to react to external events.

The combined lack of long-range sensors and poor quality dead-reckoning prevents these robots from using state of the art localization algorithms, such as simultaneous localization and mapping.
