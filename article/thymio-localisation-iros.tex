...
\caption{The error between the estimation by the localization algorithm and the ground truth, and the self confidence, on the run with kidnapping.
...
Other resolutions have a confidence of about 0.5 when the robot is localized, and the this value drops to below 0.1 after kidnapping, clearly showing that the algorithm is able to assess its status of being lost.
...
This table shows the execution duration (in second) of one step for the two algorithms with different parameters:

\vspace{.5em}

\noindent \begin{tabularx}{\columnwidth}{cccc|cccc}
\toprule
\textbf{0.64} & \textbf{1.43} & \textbf{2.15} & \textbf{2.94} & \textbf{1.97} & \textbf{2.97} & \textbf{6.08} & \textbf{12.22} \\
\multicolumn{4}{c}{Markov Localization} & \multicolumn{4}{c}{Monte Carlo Localization} \\
18 & 36 & 54 & 72 & 50k & 100k & 200k & 400k \\
\bottomrule
\end{tabularx}
...
\caption{The image, estimated trajectory and self-confidence for different 59\,$\times$\,42\,cm grayscale patterns printed on A2 papers.
...
\Fig{a2_drawings} shows the performance of the real-time localization of the Thymio using different grayscale images of 59\,$\times$\,42\,cm, printed on A2 paper sheets.
We used Markov Localization with an angular discretization of 36 and a cell resolution of 1\,cm; the localization is performed every 0.4\,s.
The robot is able to globally localize itself on all images.
The ones with more contrast lead to a more robust and faster localization, with the ones with lower contrast lead to more imprecise trajectories.
The self confidence measure is effective in assessing the quality of the localization.
