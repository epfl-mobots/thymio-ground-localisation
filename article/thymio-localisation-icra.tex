...
\begin{figure}
\includegraphics[width=.55\columnwidth]{thymio2-shadow}\hfill
\includegraphics[width=.44\columnwidth]{thymio2-dimensions}
...
The data are gathered with a high enough frequency of 100\,Hz (ground-truth) and 10\,Hz (robot) for subsequent subsampling.
...
\item \texttt{linear}: The robt simple goes straight along the x-axis of the map.
\end{itemize}
We use downsampled data to 300\,ms per time step.
...
\caption{
The error between the estimation by the localization algorithm and the ground truth, using Markov and Monte Carlo Localization, on 10 extracts from two normal runs.
For Markov Localization, the legend indicates the number of discretization steps for angles; for Monte Carlo Localization, it indicates the number of particles.
...
We see that reducing the map size does reduces the distance travelled necessary to converge, in accordance to the theory.
...
The Monte Carlo approach requires at least 100k particle for proper localization, at a computation duratoin of 2\,s per step.
While similar optimization is possible, some parts are harder to optimize, in particular the resampling step and the \textsc{ransac} step.
...
The first three, paintings from Breugel, Van Gogh and Kandinsky, allow the robot to localize in less than 10\,cm for both runs \texttt{normal\,1} and \texttt{normal\,2}.
...
While the robot localizes within 20\,cm on run \texttt{normal\,1}, it has trouble with run \texttt{normal\,2}, especially with the angle.
We attribute this degradated performance to the large areas without constrast in this painting.
The next image is a line-art drawing from a popular children comics.
Stable localization on run \texttt{normal\,1} is only achieved after travelling 150\,cm.
The reason is the low amont of information present in such images.
Nevertheless, when the trajectory encounters sufficient information, like in run \texttt{normal\,2}, accurate localization is possible.
...
This drawing allows accurate localization, after travelling 80\,cm in the first run, but only 20\,cm in the second.
...
In this paper, we have conducted an empirical evaluation of Markov- and Monte Carlo-based approaches for localizing a mobile robot with two inexpensive ground sensors on a known map.
We have shown that both approaches work, and that their performances and computational requirements are of a similar order of magnitude.
...
In addition, we have outlined a theoretical analysis method to estimate the localization performance in function of the sensors.
This can be used to take decision for the placement of sensors in a future robot if localization is a design requirement.
Finally, we have demonstrated that our localization method works on a large variety of grayscale images, from well-known paintings to drawings by children.

These contributions to the state of the art enable absolute positioning of inexpensive mobile robots in the 100\,\$ price range.
In the context of educational robotics, this opens many opportunities, such as the study of geometry, puzzles based on the physical space, and staging the creation of stories in the context of language education.
