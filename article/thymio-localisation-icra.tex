...
\begin{displaymath}
	H_{loc} = \log_2(N_{cells}) = \log_2\left(\frac{L\times W\times N_{\theta}}{h^2}\right),
\end{displaymath}
with $N_{cells}$ the number of cells, $L$ and $W$ the length and width of the environment, $h$ the size of the cell, and $N_{\theta}$ the number of discretization steps of the angle $\theta$ of the robot.
...
However in practice, there is a loss in information due to the sensor noise, characterized above with the $p_{correct}$ probability of the sensor to be correct:
\begin{displaymath}
	H_{noise} = H_{\text{b}}(1 - p_{correct}),
\end{displaymath}
where $H_{\text{b}}$ is the binary entropy function: $H_{\text{b}}(p) = -p\log_2(p) - (1-p)\log_2(1-p)$.
With $p_{correct}=0.95$ the loss in information is around 0.29\,bit per measurement.
...
We see that, with Markov localization, the correct pose is found after about 20\,cm.
...
The convergence is slower with Monte Carlo localization, excepted with 400k particles, in case it is roughly similar.
...
With Markov Localization, discretization resolutions of 36, 54 and 72 are approximately equivalent in performance, while 18 performs clearly worst, but only for angular resolution.
