...
% The first line of research solves the localization problem using several robots.
...
% This work focuses on scalability rather than absolute positioning.
...
Some approaches, such as \cite{prorok2012low, dias2013absolute, mclurkin2013low}, solve localization problem of multiple cheap robots instead of a single robot by using collaborative sensors information.

% mclurkin2013low
% not relevant

% Dias and Ventura~\cite{dias2013absolute} have used barcode on the wall of an arena and an Extended Kalman Filter to localize an e-puck robot.
% Their system reaches precision in the order of cm and 5° but requires wall around the arena to place the bar codes.
% dias2013absolute

% \cite{varveropoulos2005robot} presents a method which solves small-scale robotics localization problem using data from sonar-based range sensor.
% An occupancy algorithm based on Bayes' probabilistic model is applied and the map is discretized into 2-D grid map.

%Zug et al.~\cite{zug2011design} have developed an algorithm using an array of triangulation-based infrared distance sensors.
%Their localization algorithm uses a Kalman filter to localize the robot within a 2 by 1 meter box.
%While they do not provide experimental result, they provide a simulation estimating the error to be within 2\,cm.
In \cite{zug2011design}, a localization system is developed for supporting multi-sensor mobile robot in RoboCup Junior competitions using Kalman filter.
The localization system is applied for robot equipped with four distance sensors with limited range moving in an environment of a rectangle consisting of four walls.
...
\cite{singh2013map} uses a particle filter algorithm to solve robot localization problem with robot kidnapping using information from ultrasonic sensors.
% Neither of these papers give experimental results, and they all depend on a very specific wall arrangement.
...
The data are gathered with a high enough frequency of 100\,Hz (ground-truth) and 10\,Hz (robot) for subsequent sub-sampling with a period of 300\,ms.
...
\caption{The error between the estimation by the localization algorithm and the ground truth, using Markov and Monte Carlo Localization, on \texttt{trajectory~1} (top) and \texttt{trajectory~2} (bottom).
...
Increasing this number to 100k leads to a good localization, excepted after the robot has traveled 80; which corresponds to a long moment during which the robot rotates on spot, leading to less information acquisition, and therefore degraded precision.
