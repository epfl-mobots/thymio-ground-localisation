...
Driven by consumer products, the last decades have seen tremendous progress in electronics, motor and battery technologies.
...
In addition, dead-reckoning on these platforms is bad to non-existent, as proper wheel encoders are still too expensive.
...
For this project, only two infrared sensors directed at the ground are used (\fig{thymio}, right).
...
We use the \textsc{ros} framework\footnote{ROS (Robot Operating System): \url{http://www.ros.org/}} to synchronously record these along with sensor values and odometry information from the Thymio through a connection with Aseba.
During the dataset gathering, the robot moves on the map with a speed in the range of 3--5\,cm/s.
