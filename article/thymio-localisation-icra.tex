...
This severely limits their use, for example to create advanced educational activities.
...
Our solution to the localization of inexpensive mobile robots enables a new dimension of educational activities, strongly increasing the value of robots for education.
\end{abstract}
...
In particular, we did not find any work using only short-range intensity-based infrared sensors to provide absolute localization.
...
However, no experimental result is provided, but a simulation shows the estimated error to be within 2\, cm.
...
However, they do not provide experimental results.

Focusing on a low cost, Gutmann et al.~\cite{gutmann2013challenges} have developed a system using only 3 or 4 light detectors, and able to localize a mobile robot in a room with an accuracy of 10\,cm.
...
Part and Hashimoto~\cite{park2009approach} proposed an approach which localize a mobile robot in an environment equipped with randomly distributed passive RFID tag.
The average localization error of this method is between 7 to 9\, cm.
However, this approach is only feasible with localization problem with knowledge of starting position.
% FIXME: I would add Francesco's paper here
...
Their system employ an Extended Kalman Filter algorithm and reaches a precision in the order of cm and 5°.
...
When the cost of all robots is added, the system is far from cheap.
...
In this section, we start by presenting the generic Bayesian filter sued to estimate the pose of the robot in the environment.
Then we discuss the specificities of the Markov Localization and Monte Carlo Localization approaches for its inference.
Finally, we propose a theoretical analysis from the information theory point of view to give a lower bound on the time or distance to start achieving localization.
...
In the design of a new robot, if the aim is to have a faster localization, the greatest changes needed could be to move faster, in order to reduce redundancy in the successive measurements, or to have better sensors to reduce cross-over noise.
Setting the sensors apart also reduces the redundancy between their information but, for our specific grid size, they are sufficiently separated.
