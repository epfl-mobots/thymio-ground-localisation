...
They are now widely available at a price that makes affordable mobile robots a reality.
This opens many new opportunities, particularly in education.
Robots have been shown to effectively support teaching of knowledge such as mathematics, physics, computer science and engineering; and of skills such as problem solving and the scientific method~\cite{benitti2012explorin}.
...
In addition, dead-reckoning on programmable educational platforms is bad to non-existent, as proper wheel encoders are too expensive on platforms that must already provide a variety of sensing modalities to meet educational needs.
...
While this robot features several sensing modulatities, this project uses its two ground infrared sensors (\fig{thymio}, right) and the measurement of wheels speed.
...
The motions of the robot can be classified into two different types, namely moving forward or backward (possiblely in arcs) and rotating on spot.
To test the algorithm's ability to recover from catastrophic localization failures, we also conduct experiments with ``robot kidnapping'' by lifting the robot and moving it to another part of the map.
